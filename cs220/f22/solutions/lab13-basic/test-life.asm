; This program runs Conway's Game of Life on a 64-by-64 torus for 100
; generations, starting with the shape known as the R-penomino.
; It requires the file nextgen.asm to define the next_generation,
; and it uses the printf function from the standard C library.
; It can be assembled and linked with the build script:
;
;    ./build.sh -c test-life.asm nextgen.asm
;
; Alternatively,
;    yasm -felf64 -gdwarf2 nextgen.asm
;    yasm -felf64 -gdwarf2 test-life.asm
;    ld -g -o test-life test-life.o nextgen.o -lc --dynamic-linker /lib/x86_64-linux-gnu/ld-linux-x86-64.so.2
